...
export class FrankeOrderInvoicesService {
  ...
  constructor(
    ...
    this.userAccountFacade.get().subscribe((user) => {
      this.getOrderInvoices(user.uid).subscribe((invoices: any) => {
        if (invoices) {
  ...
  getOrderInvoices(uid: string): Observable<FrankeOrderInvoices> {
    return this.http
      .get<FrankeOrderInvoices>(
        this.occEndpointService.buildUrl('/users/' + uid + '/invoices')
      )
      .pipe(
        catchError(() => {
          this._invoicesListLoaded$.next(true);
